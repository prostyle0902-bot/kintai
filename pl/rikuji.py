...
★★ ファイル名の年月は【利用月】。そのままPL列になる ★★
    引落は利用月の2か月後。
        ...
        202608 → 8月列 →「【2026年08月利用分】」→ 振替日 2026/10/05
...
PDF_DATA = {
    ...
    # 2026-09-19 追加。8月は 21期の最終月。ファイル名が他の月と違っていて
    # 「陸事総合協同組合.pdf」。同じフォルダの CP走行明細CSV は検算に使えない
    # （CP請求鑑ではなく走行1本ずつの明細）。
    "8月":  (111065, 11106,   0, 122171, "2026/10/05"),
}

# ★Dropbox上のファイル名が規則から外れる月。rows() の「元ファイル」表示に使う
FILENAME = {"8月": "陸事総合協同組合.pdf"}
...
BANK = {"9月": 126143, "10月": 133727, "11月": 143503, "12月": 146363,
        ...
        "5月": 83354, "6月": None, "7月": None, "8月": None}
...
EXIST_21 = {"9月": 114676, "10月": 143552, "11月": 143503, "12月": 133058,
            ...
            "5月": 75777, "6月": None, "7月": None, "8月": None}
...
def rows():
    ...
    for m in MONTHS:
        ...
        yield (TAB, ROW, m, ex,
               f"買掛/21期/{yy}{int(m[:-1]):02d}月/…/業務/{FILENAME.get(m, f'0000042400_高速_請求書_{YM[m]}.pdf')}",
               note)
...
if __name__ == "__main__":
    ...
    print(f"{TAB} / {ROW}")
    print(f"請求書PDF {have}/{len(MONTHS)}本と照合、全項目一致 ✅\n")
    ...
    for tab, row, m, ex, src, note in rows():
        ...
        c = _csv_total(m)[0]
        cs = "―" if c is None else (f"{c:,}" if c == total else f"{c:,}★")
        e = EXIST_21[m]
