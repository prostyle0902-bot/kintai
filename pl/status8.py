...
SOURCES = [
    ("銀行", "千葉銀行9口座 202608", "bank/小見川支店_普通_*_202608.csv",
     ...
     "9口座とも202608が bank/ にある（2026-09-14 確認）"),
    ("銀行", "PayPay銀行", "bank/NBG_2026.csv",
     "NBG_2026.csv に8月の行", "自動（ファイルを置けば拾う）",
     "bank/NBG_202608.csv がある。bank.py は NBG*.csv を全部読むので拾われる（2026-09-14 確認）"),
    ("カード", "freeeカード", "csv/statement-2026-09.csv",
     "statement-2026-09.csv（9月ファイル＝8月利用分）", "engine.py に1行足す",
     "csv/statement-2026-09.csv があり、engine.py の一覧にも入っている（2026-09-14 確認）"),
    ...
    ("車両", "ENEOS", "eneos/ENEOS_8月.csv",
     ...
     "eneos/ENEOS_8月.csv を取り込み、eneos.py に8月を足した。税抜96,102（2026-09-14）"),
    ("請求書", "なめがたしろはとファーム", "namefa/8月.pdf",
     "なめがたしろはとふぁーむ　十三里屋9月末.pdf", "namefa.py の INVOICE に1行足す",
     "namefa.py の INVOICE に8月が入っている（2026-09-10）"),
    ("請求書", "買掛 2608月フォルダ一式", "（Dropbox）買掛/21期/2608月/",
     "各店舗フォルダのPDF", "読んで inv8.py に足す",
     "2608月フォルダに約70枚。読み終えて inv8.py に入れてある（2026-09-14 確認）"),
    ...
    ("その他", "かめや日報", "（Dropbox）", "8月ぶん", "kameya.py の MONTHS_21 に足す",
     "3点セットを読んで kameya.py の DATA に 8月 を入れた（2026-09-15）。"
     "税込売上1,291,010／消費税96,207／ロイヤリティ59,740／"
     "インフォマート10%20,040・8%347,673／経費180,530"),
