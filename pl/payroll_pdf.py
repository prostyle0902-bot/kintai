...
2026年1月は令和8年なので【R8年フォルダ版】を採る（年フォルダとして正しいほう）。
R7年フォルダのほうが更新日は新しいが、Dropboxの更新日は再アップや移動でも動くので
中身の新しさの証拠にはならない。保留リストに出しておくので確認してほしい。
"""
import collections
...
def hold_rows():
    ...
    yield ("1月", "鳥害対策課", "人件費（店長）（小林俊樹）",
           "給料一覧表-202601.pdf がDropboxに2つある。R8年フォルダ版は総支給174,970、"
           "R7年フォルダ版は251,326（差76,356）。違うのはこの1人だけ。"
           "2026年1月は令和8年なのでR8年フォルダ版を採った。"
           "R7年フォルダのほうが更新日は新しいが、Dropboxの更新日は再アップや移動でも"
           "動くので中身の新しさの証拠にはならない。どちらが正か確認してほしい")
    yield ("9月〜3月", "りゅうちゃん", "人件費（店長）",
           "9月〜7月は毎月300,000（コウノリュウジ）が21期PLシートに入っていたが、"
           "給料一覧表には載らない別経路の振込。銀行明細（PayPay・千葉銀行）には"
           "毎月ちょうど300,000の振込があり実在は確かなので、"
           "transfers.py 側で扱うか要判断。いまは入れていない")
    fb = list(fallbacks())
    if fb:
        ...
        for (no, name, tab), ms in sorted(by.items()):
            yield ("・".join(ms), tab, f"{name}（{no}）",
                   ...
                   "退職者などで名簿から消えている人。配属が違っていたら教えてほしい")
