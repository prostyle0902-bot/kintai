...
★★【21期＝このまま／22期から Airペイ も取引期間ベースにそろえる】
   利用者指示 2026-09-19「22期から直しましょう」。
   21期は社長が作っていた既存PLと同じ切り方（Airペイだけ入金日ベース）のまま。
   途中で基準を変えると月がつながらないので、期の途中では直さない。
   22期（2026年9月〜）から、Airペイも【取引期間＝暦月】で入れる。

   ☆そのために要るもの（まだ無い）:
     Airペイ管理画面の「取引集計」（AirペイQRと同じ形式。取引期間と
     合計手数料(税抜)が書いてある）を、毎月ドライブに保存してもらう。
     いまドライブにあるのは【振込明細CSV】だけで、これは
         振込日／利用年月日／決済ブランド／下4桁／支払区分／決済金額（円）
     の明細と、末尾の合計行
         売上合計金額／売上手数料／振込手数料／振込金額
     しか無い。**1件ごとの手数料が載っていない**ので、利用年月日で month を
     切り直しても手数料を割り直せない（ブランドごとに料率が違うため）。
   ☆取引集計が手に入らない場合の次善策:
     複数月ぶんの振込明細CSVから決済ブランド別の料率を逆算し、
     利用年月日で割り直す。ただし端数処理が分からないので【概算】になる。
     やるなら概算である旨を必ずメモに残すこと。
...
DATA = {
    ...
    ("タコとハイボール", "支払手数料（AirPay）", "8月"): (
        ...
        "入金日 8/5・8/17・8/25（利用 7/20〜8/18）"),
    # ★2026-09-19 追加。既存21期PLにはりゅうちゃんの8月が無かった（11か月ぶんだけ）。
    #   ドライブに明細があったので、書類から入れた1件。
    ("りゅうちゃん", "支払手数料（AirPay）", "8月"): (
        24306, "取引集計（Airペイ）/振込明細-052989708086-20260910.csv",
        "CSV最終行の合計 856,510／手数料24,306／振込832,204。"
        "入金日 8/5・8/17・8/25（利用 7/20〜8/18）。"
        "★既存21期PLの りゅうちゃん「支払手数料（AirPay）」は8月が空だった"
        "（9月〜7月の11か月ぶんだけ）。ドライブの明細で裏を取って足した"),
