...
COMMON_COGS = ["仕入（鹿島食品8%）","仕入（鹿島食品10%）","仕入（藤原ストア）","仕入（平洋酒店）",
               ...
               "仕入（やまなか）","仕入（その他）"]
# 会議費は接待交際費のすぐ下。1件5,000円以下の飲食費をここへ回す（利用者指示 2026-08-22）
COMMON_SGA = ["人件費（店長）","人件費（アルバイト）","法定福利費","旅費・交通費","接待交際費","会議費",
...
EXTRA_COGS = {
 ...
 # 仕入（東総食肉）は 2026-09-01 新設（利用者指示「竜ちゃんに作成」）。
 # 2509月の請求書に「大衆酒場」あて 国産豚肩ロースブロック 4,197（8%）があった
 "りゅうちゃん": ["仕入（鹿島食品）","仕入（東総食肉）","仕入（藤原ストアー値引き）","仕入（現金）"],
 # 仕入（新場水産）は 2026-08-21 新設（利用者指示「その他の下に」＝仕入（その他）の下）
 # 仕入（お米）は 2026-08-22 新設。ヒラノ　タカシへの振込＝もも焼きで買っている
 # お米（利用者確認）。軽減8%。norow.py 参照
 "もも焼きJAPAN": ["仕入（新場水産）",
              "仕入（鹿島食品）","仕入（山食)","仕入（鈴喜）","仕入（日本食研）",
              "仕入（東京さえき）","仕入（京兼醸造）","仕入（東総食肉）",
              "仕入（お米）","仕入（現金）"],
 ...
 "神栖横丁": ["仕入（ビーエム）","ALSOK","ウゴーク","クリーズコネクト（ゴミ）","ダスキン",
          ...
          # 兵庫衛は 2026-09-01 新設（利用者指示「横丁に作成」）。出演代
          "ガキゲン","居酒屋ドリーム","鉄板焼次郎","ツマギアンズ","兵庫衛",
          # 以下5件は既存21期PLにあるのを写した（2026-08-21）。既存の並び順に合わせている
          "鹿島アントラーズ","シオン","ALホーム","セキネネオン","タカギ",
          # 2608月の請求書で新設。アントラーズビアガーデンのギャラ代。
          # ガキゲン〜ツマギアンズ・鹿島アントラーズと同じビアガーデン関係なので同じ区分にした
          "かりすまーず",
          # ※井上防災工事は 2026-09-10 にここへ作ったが、利用者判断 2026-09-11
          #   「点検かな、修繕費で」により EXTRA_SGA の「修繕費」に移した。
          # ※Cqreeはここに作りかけたが、韓国酒場ハナに既に「仕入（CQREE）」行があるので取りやめ
          # スポット（Locoty保守）は 2026-09-01 新設（利用者指示「按分にしましょう」）。
          # 株式会社SPOTのLocoty WEBサポート年間保守料98,000を12か月に割る。inv11.py 参照
          "スポット（Locoty保守）",
          "その他、仕入（小口）"],
 # 外注費（グリーンフィールド）は 2026-09-01 新設（利用者指示「Green Field→鳥害対策」）
 "鳥害対策課": ["仕入（日本鳩対策センター）","外注費（西尾レントオール）",
           "外注費（グリーンフィールド）","外注費（プロエンジニア）",
 ...
 "業務課": ["仕入（ビーエム）","仕入（グローバルクリーン）","仕入（ニッセーデリカ）",
         ...
         "西尾レントオール","総合環境分析","ササモト技研",
         # 小林塗料は 2026-09-02 新設（利用者指示「小林塗料は業務部に作って」）。自動車補修用塗料
         "小林塗料",
         "トヨタファイナンス（燃料）",
         "陸自総合（高速代）","その他、仕入（小口）"],
 ...
 "本部": ["JCBカード","三井住友カード","JBQ","TMAP","オニオン新聞","ジョイント","スギヤマ","セコム",
        ...
        # 廣瀬造園は 2026-09-01 新設（利用者指示「本部に作って」）。草抜き・清掃
        # 出張旅費は 2026-09-01 新設（利用者指示「新しく項目作って」）。
        # 2026/08/12 に本体口座から4人へ振り込んだ7月の出張旅費。norow.py 参照
        # ★イード（JTOPIA）は一度ここに行を作ったが、利用者確認 2026-09-01
        #   「イードは消耗品」で 業務課「消耗品費」へ移したので消した
        "セキショウホンダ","ウェイブ","廣瀬造園","出張旅費","その他"],
}
EXTRA_SGA = {
 "りゅうちゃん": [],
 # 2026-08-22 新設（利用者指示「行を作成して」）。norow.py 参照
 # 外注費（MEG design office）… イベントサポート業務の請求書3枚
 # ★ヒラノ　タカシ は「もも焼きで買ってるお米」（利用者確認 2026-08-22）なので
 #   販管費ではなく売上原価。EXTRA_COGS の「仕入（お米）」に移した。
 "もも焼きJAPAN": ["消耗品費（関彰商事）","その他経費（華まる支払い）",
              "外注費（MEG design office）"],
 "韓国酒場ハナ": ["リース料",
             # 2026-08-22 新設。12月の片野商店67,210は工事費（利用者確認）
             "工事費"],
 ...
 "神栖横丁": ["通信費（USEN）","通信費（ダゾーン）","通信費（有線ネット）","通信費（郵便関係・その他）",
          ...
          "広告宣伝費（LINE）",
          # 広告宣伝費は 2026-09-02 新設（利用者指示 セキネネオンの差額100,000について「広告宣伝費」）。
          # 看板まわりの宣伝費の受け皿。LINE・共通宣伝費とは別物
          "広告宣伝費",
          # 修繕費は 2026-09-11 新設（利用者判断「点検かな、修繕費で」）。
          # 井上防災工事（消防設備の点検・誘導灯用LEDランプ交換）の受け皿。
          # 業務課にも同名の行があるので行名はそろえてある
          "修繕費"],
 # 支払手数料（クレジット決済）は 2026-09-13 新設（利用者指示
 #   「これ、手数料を鳥害対策課に入力欄作成し、入れておいて」）。
 #   カメイ・テン・サービスのお支払計算書（銀行明細/21期/お支払計算書_*.csv）の
 #   加盟店手数料の受け皿。AirPay・AirPayQRとは別系統なので行を分けてある。kamei.py 参照
 "鳥害対策課": ["支払手数料（クレジット決済）"],
 "本部": ["人件費　社長","人件費　純子","法定福利費","消費税","労働保険料","旅費・交通費",
        "広告宣伝費","接待交際費","会議費","地代家賃（145,000÷3）","通信費","事務消耗品費","租税公課",
        "減価償却費","水道","電気代　地域創生","電気代　東京電力","ガス代","保険料（損保ジャパン）",
        # JCB明細を取引先別に割るため新設（利用者指示 2026-08-20）
        "福利厚生費","ソフトウェア利用料",
        # 会費類の受け皿として新設（利用者指示 2026-08-20）。shokaihi.py 参照
        "諸会費",
        # 外注費は 2026-09-02 新設（利用者指示「ALホームも本部外注費で」）。
        # 本社（小見川）のエアコン取付工事など
        "外注費",
        "雑費","その他経費"],
 "業務課": ["事務消耗品費","諸会費（日本ビルメン）","修繕費（リペアボックス）",
         # 修繕費は 2026-09-02 新設（利用者指示「業務課で修繕費に入れといて」）。
         # 空調の修理・点検（三菱電機システムサービス／日本キャリア）の受け皿。
         # 「修繕費（リペアボックス）」は取引先を名前に持つ別の行なので分けた
         "修繕費"],
}


# ---- 使っていない行の削除（利用者指示 2026-08-22）--------------------------
# 「各店舗、事業部で使ってない行は削除しよう。22期も。ただし販管費の行は触らない」
# ★触るのは【売上原価】の行だけ。販管費（COMMON_SGA / EXTRA_SGA）は一切触らない。
#
# 21期の実データで1セルも使われなかった売上原価の行（2026-08-22 実測）。
# ★もも焼きJAPAN・韓国酒場ハナの「仕入（現金）」は削除対象から外した（2026-08-25）。
#   店舗口座のATM引出を現金仕入として入れることにしたため（genkin.py）。
#   焼きたて屋・さわら十三里屋は引出が無いので削除したまま。
UNUSED_COGS = {
    "りゅうちゃん": ["仕入（鹿島食品8%）", "仕入（鹿島食品10%）", "仕入（現金・米）"],
    "もも焼きJAPAN": ["仕入（鹿島食品8%）", "仕入（鹿島食品10%）", "仕入（藤原ストア）",
                  "仕入（平洋酒店）", "仕入（六角堂）", "仕入（現金・米）", "仕入（ネット食材）",
                  "仕入（やまなか）"],
    "韓国酒場ハナ": ["仕入（鹿島食品8%）", "仕入（鹿島食品10%）", "仕入（藤原ストア）",
                "仕入（平洋酒店）", "仕入（六角堂）", "仕入（現金・米）", "仕入（ネット食材）",
                "仕入（やまなか）", "仕入（その他）"],
    "さわら十三里屋": ["仕入（鹿島食品8%）", "仕入（鹿島食品10%）", "仕入（藤原ストア）",
                 "仕入（平洋酒店）", "仕入（六角堂）", "仕入（現金・米）", "仕入（ネット食材）"],
    "タコとハイボール": ["仕入（鹿島食品8%）", "仕入（鹿島食品10%）", "仕入（藤原ストア）",
                  "仕入（平洋酒店）", "仕入（六角堂）", "仕入（現金・米）", "仕入（ネット食材）",
                  "仕入（やまなか）"],
    "焼きたて屋": ["仕入（鹿島食品8%）", "仕入（鹿島食品10%）", "仕入（藤原ストア）",
               "仕入（平洋酒店）", "仕入（六角堂）", "仕入（現金・米）", "仕入（ネット食材）",
               "仕入（やまなか）", "仕入（現金）"],
    "神栖横丁": ["仕入（鹿島食品8%）", "仕入（鹿島食品10%）", "仕入（藤原ストア）",
              "仕入（平洋酒店）", "仕入（六角堂）", "仕入（現金・米）", "仕入（ネット食材）",
              "仕入（やまなか）", "仕入（その他）", "東総食肉センター"],
    # ★「外注費（ロープワーク）」は 2026-09-16 に UNUSED から外した。
    #   21期のロープワークは従来すべて業務課に入れていたので未使用にしてあったが、
    #   2608月フォルダに遅れて届いた7月分（ヒューマンスクエア四街道ピアンテの
    #   鳥害対策 60,000）を、利用者指示「鳥害対策課へ。今回分だけ」で
    #   鳥害対策課に入れることになった（inv7.py）。過去分は業務課のまま。
    "鳥害対策課": ["仕入（鹿島食品8%）", "仕入（鹿島食品10%）", "仕入（藤原ストア）",
               "仕入（平洋酒店）", "仕入（六角堂）", "仕入（現金・米）", "仕入（ネット食材）",
               "仕入（やまなか）", "仕入（その他）", "その他、仕入（小口）"],
    "業務課": ["仕入（鹿島食品8%）", "仕入（鹿島食品10%）", "仕入（藤原ストア）",
             "仕入（平洋酒店）", "仕入（六角堂）", "仕入（現金・米）", "仕入（ネット食材）",
             "仕入（やまなか）", "仕入（その他）"],
    "本部": ["テイクアクション", "ニッサン", "バイオラボ", "鹿島アントラーズ", "ピープルフォレスト"],
}

# 鹿島食品の扱い。利用者指示「22期は8％10％に統一」。
#   21期 … 実データは一括の1行に入っている（税率で分かれていない）。一括行を残す。
#   22期 … 8%と10%の2行を残し、一括行を落とす。最初から税率で分けて入れる。
# 値: 21期で使っている一括行の名前（神栖横丁だけ「鹿島食品」で括弧が無い）
KASHIMA_LUMP = {
    "りゅうちゃん": "仕入（鹿島食品）", "もも焼きJAPAN": "仕入（鹿島食品）",
    "韓国酒場ハナ": "仕入（鹿島食品）", "さわら十三里屋": "仕入（鹿島食品）",
    "タコとハイボール": "仕入（鹿島食品）", "焼きたて屋": "仕入（鹿島食品）",
    "神栖横丁": "鹿島食品",
}
_KASHIMA_SPLIT = ["仕入（鹿島食品8%）", "仕入（鹿島食品10%）"]


def dropped_cogs(tab, period=None):
    """その期で落とす売上原価の行。"""
    period = period or PERIOD
    drop = set(UNUSED_COGS.get(tab, []))
    if period != "21期" and tab in KASHIMA_LUMP:
        drop -= set(_KASHIMA_SPLIT)          # 22期は8%/10%を残す
        drop.add(KASHIMA_LUMP[tab])          # かわりに一括行を落とす
    return drop
