...
# ★神栖横丁「通信費（ダゾーン）」の 6月・7月 各2,909 は【二重計上】だった
#   （2026-09-19 に見つけた）。同じ DAZN の支払いが freeeカード明細にもあり、
#   engine.py の取引先マスタが「ダゾーン」→ 神栖横丁「その他経費」に入れている。
#     カード明細: 2026-06-20／07-20／08-20 いずれも 税込3,200（税抜2,909）
#   既存21期PLの「通信費（ダゾーン）」から写すと、同じ3,200を2回数えることになる。
#   利用者指示 2026-09-19「freeeカードCSVを基準に」に従い、既存PL側を写さない。
#   ★行の置き場所（その他経費のままでよいか、通信費（ダゾーン）に移すか）は別の話。
#     いまはカード明細の入り先＝その他経費のまま。移すなら engine.py のマスタを直す。
for _m in ("6月", "7月"):
    SKIP_CELL[("神栖横丁", "通信費（ダゾーン）", _m)] = (
        "同じ DAZN の支払いが freeeカード明細にもあり（税込3,200・税抜2,909）、"
        "engine.py が神栖横丁「その他経費」に入れている。"
        "既存PLからも写すと二重計上になるので写さない（利用者指示 2026-09-19）")
