    ...
    for ym in months_with_data():
        ...
        for name, (tab, urirow, zeirow) in STORES.items():
            if name == "焼きたて屋":
                ...
                # ★焼きたて屋の売上は かめや の合計精算書がすでに入れている月がある。
                #   2608月は FC月間売上集計一覧表（税込1,291,010／消費税96,207）と
                #   かめや合計精算書（同額）が1円まで一致した。二重計上になるので
                #   かめやがある月はかめやを正とし、ここでは出さない。
                #   （食い違ったら気づけるように下で突き合わせている）
                if m in kameya.DATA:
                    k = kameya.DATA[m]
                    assert (inc, tax) == (k["売上税込"], k["消費税"]), (
                        f"{m} 焼きたて屋: エアレジ {inc:,}／{tax:,} と "
                        f"かめや精算書 {k['売上税込']:,}／{k['消費税']:,} が食い違う")
                    continue
