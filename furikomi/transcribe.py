...
class Tab:
    ...
    def is_total_row(self, r):
        ...
        ★以前は「A〜V列のどこかに『合計』の文字があれば合計行」としていたが、
          これだと**各セクションの最終データ行が使えなくなる**バグがあった。
          合計行のラベル「合計」はT列で2行ぶん結合されていて、値は上のセル
          （＝1行上のデータ行）に入るため。実際 4月タブは行67のF/G列に
          利用者自身が cqree 72,199 を書いていて、行68のセクション計
          546,807 にもちゃんと入っている（行59〜67の和と一致）。
          2026-09-07 に Cqree の請求書が「空き行が無い」で保留になって発覚。

        いまの判定: **セクションの最終行＝合計行**。21期シートは12か月とも
        月末5-26／10日27-48／小見川49-58／自動59-68 で、最終行26・48・58・68が
        実際の合計行と1行も違わないことを確認済み。
        A・B列に「合計」の文字がある行も従来どおり避ける。
        """
        for key, (a, b) in self.sections.items():
            if a <= r <= b:
                if r == b:
                    return True
                break
        return any(str(self.cell(r, c)).strip() == "合計" for c in (0, 1))
