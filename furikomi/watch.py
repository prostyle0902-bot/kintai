...
--- ★「★毎月ここに入れる」フォルダ（2026-09-19 新設）--------------------
利用者が毎月ダウンロードする元データ（銀行・freeeカード・JCB・board・
エアレジ・かめや・会計明細）は、全部このフォルダに入れてもらう。
入れる場所がバラバラで忘れやすい、という相談から作った。

    ★毎月ここに入れる/
       01_千葉銀行・PayPay銀行 ／ 02_freeeカード ／ 03_JCB・三井住友カード
       04_board売上 ／ 05_エアレジ売上 ／ 06_かめや（焼きたて屋）
       07_会計明細 ／ 99_その他・わからないもの
       はじめに読む.txt ／ 今月の受け入れ状況.txt

取り込んだら**本来の置き場所へ移す**（Dropboxのmove。file_idは変わらないので
台帳はそのまま使える）。フォルダが空＝全部取り込み済み、が利用者への合図。
「今月の受け入れ状況.txt」は pl/monthly_status.py が本文を作る。
★create_file は同名があると ALREADY_EXISTS で落ちる（上書きにならない）。
  書き換えるときは delete → create_file の順にすること（2026-09-20 実測）。
...
# 見張るフォルダ。増やすときはここに足す
WATCH = [
    # (0) ★利用者が毎月の元データを入れる場所（2026-09-19 新設）
    #     ここに何か入っていたら【未処理】。取り込んだら本来の置き場所へ移す。
    #     サブフォルダごとではなく親フォルダ1つを見張れば足りる
    #     （folder_size は中のファイルのサイズ合計なので、どの階層に入っても増える）。
    #     ★「はじめに読む.txt」と「今月の受け入れ状況.txt」が常駐しているので、
    #       空のときでも 0 にはならない。基準値と同じなら新着なし。
    f"{NS}//★毎月ここに入れる",
    # (A) 振込一覧 … 請求書
    f"{NS}//買掛/21期/2607月",
    f"{NS}//買掛/21期/2608月",
